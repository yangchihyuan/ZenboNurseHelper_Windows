...
#include <dirent.h>  //This file is available on Ubunut, but unavailabe on Windows. There is a third-party dirend.h availalbe at https://github.com/tronkko/dirent
...
static void _mkdir(const char *dir) {
        ...
        for(p = tmp + 1; *p; p++)
                if(*p == '/') {
                        *p = 0;
#ifdef __linux__                         
                        mkdir(tmp, S_IRWXU);        //The mkdir() function is in <sys/stat.h> and unavailable on Windows.
#endif
                        *p = '/';
                }
#ifdef __linux__                         
